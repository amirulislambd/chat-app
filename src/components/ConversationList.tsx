...
export default function ConversationList({
  ...
  return (
    <div
      className={`relative sticky top-0 flex h-full min-h-0 flex-col bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-700 w-full md:w-[var(--sidebar-width)] flex-shrink-0 ${className}`}
      style={{ "--sidebar-width": `${width}px` } as React.CSSProperties}
    >
      <button
        type="button"
        onMouseDown={onResizeStart}
        className="absolute right-0 top-0 z-30 hidden h-full w-1 cursor-col-resize bg-transparent transition-colors hover:bg-blue-400/70 md:block"
          ...
          {showProfileMenu && (
            <div className="absolute top-11 left-0 z-50 w-56 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
              <div className="p-4 bg-gradient-to-br from-green-50 to-teal-50 dark:from-gray-700 dark:to-gray-700 border-b border-gray-100 dark:border-gray-600">
                <div className="w-14 h-14 rounded-full bg-gradient-to-br from-green-400 to-teal-500 flex items-center justify-center text-white font-bold  mb-2 shadow-md">
                  {initials}
                </div>
                <p className="text-sm font-semibold text-gray-900 dark:text-white text-center truncate">
                  {currentUser?.name}
                </p>
                <p className="text-xs text-gray-500 dark:text-gray-400 text-center truncate mt-0.5">
                  {currentUser?.phone}
                </p>
              </div>
              <button
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <div className="p-8 text-center flex flex-col items-center justify-center h-full">
            ...
            <button
              onClick={() => setIsNewChatModalOpen(true)}
              className="bg-green-500 hover:bg-green-600 text-white font-medium py-2 px-4 rounded-full text-sm transition-colors"
            >
              Start a Chat
            </button>
          </div>
          ...
          <ul>
            {conversations.map((conv) => {
              ...
              return (
                <li
                  key={conv._id}
                  onClick={() => handleSelectConversation(conv._id, conv)}
                ...
                >
                  ...
                  <div className="ml-3 flex-1 min-w-0 border-b border-gray-100 dark:border-gray-700/50 pb-3">
                    <div className="flex justify-between items-baseline">
                      <h3
                        className={`text-sm font-semibold truncate ${unread > 0 ? "text-gray-900 dark:text-white" : "text-gray-800 dark:text-gray-100"}`}
                      >
                        {name}
                      </h3>
                      <span
                        className={`text-[11px] flex-shrink-0 ml-2 ${unread > 0 ? "text-green-600 dark:text-green-400 font-semibold" : "text-gray-400 dark:text-gray-500"}`}
                      >
                    ...
                    <p
                      className={`text-xs truncate mt-0.5 ${unread > 0 ? "font-semibold text-gray-700 dark:text-gray-200" : "text-gray-500 dark:text-gray-400"}`}
                    >
